...
class VulkanApplication {
	...
	private:
		...
		void pickPhysicalDevice() {
			...
			// if we have 1 or more devices, we can retrieve a list of physical devices
		...
		void createImageViews() {
			// resize the list to fit all the image views we'll be creating
	...
	// we ne need to wrap the shader code into the VkShaderModule object
	...
	bool isDeviceSuitable(VkPhysicalDevice device) {
		// for current purposes, we only need a device that has a queue family for graphics
		...
		// if a graphics queue exists it will return true
	...
	VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
		for (const auto& availableFormat : availableFormats) {
			// return the format that has an RGB color format
			if (availableFormat.format == VK_FORMAT_B8G8R8A8_SRGB && availableFormat.colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR) {
				return availableFormat;
			}
		}
		// if we dont get our desired color format than the first is usually suitable
